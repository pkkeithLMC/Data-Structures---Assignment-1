...
# SKILL: Loop through every dictionary and accumulate new result dictionaries.
...
def rank_activities(activity_list, interests):
    ...
    # SKILL: Sort a list of dictionaries by a selected dictionary value.
    # SKILL: Use a lambda as a short key function and reverse=True for descending order.
    ranked.sort(key=lambda result: result["match_count"], reverse=True)
    return ranked
...
if recommendations:
    for result in recommendations:
        match_text = ", ".join(sorted(result["matching_features"]))
        print(f"{result['name']}: {result['match_count']} match(es) [{match_text}]")
else:
...
# 5. How are ties handled by the sort?
